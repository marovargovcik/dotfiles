...
```sh
for s in dbus elogind iwd chronyd bluetoothd avahi-daemon cupsd cups-browsed \
         snapperd cronie; do
...
## 5. Networking (iwd + DNS)

iwd does its own DHCP and hands the nameservers to resolvconf.
**`/etc/iwd/main.conf`**:
...
**`/etc/resolvconf.conf`** — Cloudflare pinned over whatever the router
advertises; drop `name_servers` to fall back to DHCP.
...
Apply: `sudo sv restart iwd && sudo resolvconf -u`.
